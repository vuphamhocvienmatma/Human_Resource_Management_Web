...
namespace Human_Resource_Management_Libraly.Cryptography
{
    public static class PBKDF2Provider
    {
        ...
        const int constIterations = 100000; // The number of times to encrypt the password - change this
        const int constSaltByteSize = 64; // the salt size - change this
        const int constHashByteSize = 256; // the final hash - change this
        ...
        public static string PBKDF2_Keccak_GetHash(string password, string saltAsBase64String, int iterations, int hashByteSize)
        {
            var saltBytes = Convert.FromBase64String(saltAsBase64String);

            var hash = PBKDF2_KeccakDigest_GetHash(password, saltBytes, iterations, hashByteSize);
        ...
        public static byte[] PBKDF2_KeccakDigest_GetHash(string password, byte[] salt, int iterations, int hashByteSize)
        {
            var pdb = new Pkcs5S2ParametersGenerator(new Org.BouncyCastle.Crypto.Digests.KeccakDigest());
        ...
        {
            byte[] testHash = PBKDF2_KeccakDigest_GetHash(password, saltBytes, iterations, hashByteSize);
